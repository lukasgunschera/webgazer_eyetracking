...
qualitrics01 <- read.csv('eyeTrackingDemo_May 31, 2021_01.37.csv', header = TRUE)
...
allFiles <-  list.files(path = '/Users/lukasgunschera/Documents/UvA/Intern/data/dat_clean',
                      pattern = "pav.csv",
                      full.names = TRUE,
                      recursive = TRUE)
...
yPilot <- list()
yPilot <- lapply(1:length(allFiles), function(f){
  
  print(f)
  
  dat01 <-  read.csv(allFiles[f], header = TRUE, na.strings=c(""," ","NA"))
  ...
  if(!'response_type' %in% colnames(dat02)){
    ...
    dat03 <- data_frame(
      ...
      consent = dat02$Q3, webcam = dat02$Q4, webcam_type = dat02$Q19, attracted = dat02$Q17,
    ...
    var_label(dat03$attracted) <- "sexual preference"
